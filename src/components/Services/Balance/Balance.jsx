...
mb-4'>{item.benefitTitle}</h1>
                ...
                <section className='flex gap-10 flex-wrap justify-center'>
                  ...
                  <div className='border-2 h-[300px] md:w-[264px] p-8 relative overflow-hidden mx-4 my-8'>
                    <h1 className='text-2xl text-secondary py-2 font-bold'>{item.fitnessCardTitle}</h1>
                    <p>{item.fitnessDescription}</p>
                    <div className="absolute inset-0 hover:bg-gradient-to-t from-one to-transparent"></div>
                  </div>

                  <div className='border-2 h-[300px] md:w-[264px] p-8 relative overflow-hidden mx-4 my-8'>
                    <h1 className='text-2xl text-secondary py-2 font-bold'>{item.nutritionalCardTitle}</h1>
                    <p>{item.nutritionalDescription}</p>
                    <div className="absolute inset-0 hover:bg-gradient-to-t from-one to-transparent"></div>
                  </div>

                  <div className='border-2 h-[300px] md:w-[264px] p-8 relative overflow-hidden mx-4 my-8'>
                    <h1 className='text-2xl text-secondary py-2 font-bold'>{item.getCardTitle}</h1>
      ...
      <section>
        {
          trainers.filter(trainer => trainer.category === "FITNESS")
            .slice(0, 1)
            .map(trainer => (
              <div key={trainer._id}>
                ...
                <div className='flex gap-8 flex-col lg:flex-row'>
                  ...
                  <div>
                    ...
                    <div className=" py-4 ">
                      <Link href={`/services/hire-form/${trainer._id}`}>
